...
LastEditTime: 2023-09-21 11:19:23
'''
import os
import cv2
import time
...
def cam1_operation(num):
    ...
    if dev_info_list[0].get("device_class") == gx.GxDeviceClassList.USB2:
        ...
        # cam1.TriggrMode.set(gx.GxSwitchEntry.ON)
        cam1.TriggerSource.set(gx.GxTriggerSourceEntry.SOFTWARE)
    cam1.stream_on()
...
def main():
    ...
    global image_list
    image_list = []
    # initialize the turntable machine
    open_zt()
    set_zt_initial()
    # initialize the DMD
    DMD_initialize()
    # initialize the Camera device1, device2 and calibration
    # Camera_initialize()
    # create the threads
    '''
        ...
        先测试thread1,2的可实现性,然后再加入相机2的控制程序
    '''
    ...
    # release the sources
    # free_sources()
    close_zt() # 单独测试cam1和zt的联动的时候free资源
    # image_list processing
    print(len(image_list))  # 先验证一下他是正确的获取了多少个图像，第二部验证cam2_operation的代码功能验证部分
